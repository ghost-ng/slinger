...
class SlingerController:
    ...
    def __init__(self, cmdline):
        ...
        self.proc = pexpect.spawn(
            parts[0], parts[1:], env=env, encoding="utf-8", echo=False, timeout=30
        )
...
def main():
    ...
    while True:
        ...
        if step is None:
            ...
            while True:
                ...
                if not isinstance(checks, list) or any(
                    not isinstance(chk.get("narrative"), str)
                    or not chk["narrative"].strip()
                    or not isinstance(chk.get("command"), str)
                    or not chk["command"].strip()
                    for chk in (checks or [])
                ):
                    print(
                        "⚠️ Invalid verify_objective response—missing or empty fields.",
                        file=sys.stderr,
                    )
                    ...
                    messages.append(
                        {
                            "role": "user",
                            "content": "The verify_objective response must be JSON with boolean 'complete' and an array 'checks', "
                ...
                for chk in checks:
                    ...
                    out = ctrl.send(cmd)
                    history.append(
                        {"phase": "verify", "narrative": nar, "command": cmd, "output": out}
                    )
                    print(f"\n[VERIFY] {nar}\n[CMD   ] {cmd}")
                    print("⤷", "\n    ".join(out))
                    messages.append({"role": "assistant", "content": json.dumps(chk)})
                    messages.append(
                        {"role": "user", "content": "Verify output:\n" + "\n".join(out)}
                    )
        ...
        if not isinstance(cmd, str) or not cmd.strip():
            ...
            messages.append(
                {
                    "role": "user",
                    "content": f'The last response had an invalid or empty `command` for the step: "{nar}". '
        ...
        history.append({"phase": "execute", "narrative": nar, "command": cmd, "output": out})
    ...
    assessment_messages = [
        {
            "role": "system",
            "content": "You are an expert penetration tester reviewing a completed run. "
        ...
        {
            "role": "user",
            "content": f"The following is the history of actions taken:\n```\n{history_json}\n```",
        },
    ]
    resp = API.chat.completions.create(
        model=OPENAI_MODEL, messages=assessment_messages, temperature=0.0
    )
