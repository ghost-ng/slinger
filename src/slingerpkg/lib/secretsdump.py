        ...
        try:
            if self.__remoteName.upper() == "LOCAL" and self.__username == "":
                ...
                if self.__ldapFilter is not None:
                    logging.info(
                        "Querying %s for information about domain users via LDAP" % self.__domain
                    )
                    try:
                        self.ldapConnect()
            ...
            if self.__useKeyListMethod is True:
                try:
                    ...
                    logging.error(
                        "Something went wrong with the Kerberos Key List approach.: %s" % str(e)
                    )
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="smbexec",
        help="Remote exec " "method to use at target (only when using -use-vss). Default: smbexec",
    )
    ...
    group.add_argument(
        "-history", action="store_true", help="Dump password history, and LSA secrets OldVal"
    )
