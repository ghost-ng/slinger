# https://github.com/vincd/samdumpy/blob/master/samdum.py
...
NK_HDR = namedtuple(
    "NK_HDR",
    "id type t1 t2 unk1 parent_off subkey_num unk2 lf_off unk3 value_cnt value_off sk_off classname_off unk41 unk42 unk43 unk44 unk5 name_len classname_len key_name",
)
...
class RegHive(object):
    ...
    def __read_nk(self, offset):
        n = NK_HDR._make(
            unpack("hhiiiiiiiiiiiiiiiiihhs", self.__base[offset + 4 : offset + 4 + 77])
        )
...
def get_hashes(h, sam_key):
    ...
    # create dict of username rid lmhash nthash
    hash_table_list = []
    ...
    for regkeyname in h.regEnumKey(n):
        ...
        hash_table["Username"] = username
        hash_table["RID"] = rid

        lmhash = get_hash(sam_key, user_account, rid, 13, almpassword)
        # if lmhash empty use default
        hash_table["LMHash"] = lmhash if lmhash else empty_lm
        nthash = get_hash(sam_key, user_account, rid, 14, antpassword)
        # if nthash empty use default
        hash_table["NTHash"] = nthash if nthash else empty_nt
        # hash_table['NTHash'] = get_hash(sam_key, user_account, rid, 14, antpassword)

        # print(hash_table)
        hash_table_list.append(hash_table)

    return hash_table_list
