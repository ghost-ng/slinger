...
class scm:
    ...
    def enum_services(self, args=None):
        ...
        if force or len(self.services_list) == 0:
            ...
            print_log(
                tabulate(
                    new_list,
                    headers=["ID", "Service Name", "Display Name", "State"],
                    tablefmt="psql",
                )
            )
        ...
        for i in range(len(response)):
            ...
            self.services_list.append(
                [
                    count,
                    response[i]["lpServiceName"][:-1],
                    response[i]["lpDisplayName"][:-1],
                    state_str,
                ]
            )
            count += 1
        ...
        print_log(
            tabulate(
                new_list, headers=["ID", "Service Name", "Display Name", "State"], tablefmt="psql"
            )
        )
    ...
    def view_service_details(self, service_arg):
        ...
        try:
            ...
            if resp["ErrorCode"] == 0:
                print_log(
                    "TYPE              : %2d - " % resp["lpServiceConfig"]["dwServiceType"], end=" "
                )
                ...
                print_log("")
                print_log(
                    "START_TYPE        : %2d - " % resp["lpServiceConfig"]["dwStartType"], end=" "
                )
                ...
                print_log(
                    "ERROR_CONTROL     : %2d - " % resp["lpServiceConfig"]["dwErrorControl"],
                    end=" ",
                )
                if resp["lpServiceConfig"]["dwErrorControl"] == 0x0:
                    ...
                    print_log("UNKNOWN")
                print_log(
                    "BINARY_PATH_NAME  : %s" % resp["lpServiceConfig"]["lpBinaryPathName"][:-1]
                )
                print_log(
                    "LOAD_ORDER_GROUP  : %s" % resp["lpServiceConfig"]["lpLoadOrderGroup"][:-1]
                )
                ...
                print_log(
                    "SERVICE_START_NAME: %s" % resp["lpServiceConfig"]["lpServiceStartName"][:-1]
                )
    ...
    def create_service(self, args):
        ...
        try:
            ...
            response = self.dce_transport._create_service(
                service_name, bin_path, start_type, display_name
            )
            if response["ErrorCode"] == 0:
