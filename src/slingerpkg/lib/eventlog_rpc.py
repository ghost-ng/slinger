...
class EventLogRPC:
    ...
    def connect(self):
        ...
        try:
            ...
            # Set authentication
            if self.ntlm_hash:
                lmhash, nthash = self.ntlm_hash.split(':') if ':' in self.ntlm_hash else ('', self.ntlm_hash)
    ...
    def query_events(self, log_name, count=10, start_record=None, read_direction='forward'):
        ...
        try:
            ...
            try:
                ...
                print_debug(f"Reading {count} events from {log_name}, starting at record {start_record}")
            ...
            while offset < len(buffer):
                ...
                try:
                    ...
                    if source_end > strings_offset:
                        source_name = buffer[strings_offset:source_end].decode('utf-16le', errors='ignore')
                        strings_offset = source_end + 2
                    
                    # Computer name follows
                    comp_end = buffer.find(b'\x00\x00', strings_offset)
                    if comp_end > strings_offset:
                        computer_name = buffer[strings_offset:comp_end].decode('utf-16le', errors='ignore')
                except:
                    pass
...
class EventLogRPCManager:
    ...
    def execute_eventlog_operation(self, operation, method='auto', **kwargs):
        ...
        if not method_available:
            raise Exception(f"Method '{method}' not available. Available: {[m[0] for m in available]}")
