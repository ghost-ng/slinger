...
class ErrorClassifier:
    ...
    def _create_error_info(
        cls, category: ErrorCategory, message: str, retry_count: int
    ) -> ErrorInfo:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    ...
                    print(
                        f"Retry {attempt + 1}/{self.max_retries} after {delay:.1f}s: {error_info.suggested_action}"
                    )
                    time.sleep(delay)
...
class ResumeDownloadRecovery:
    ...
    def __init__(self, smb_client, max_retries: int = 5):
        ...
        self.connection_recovery = ConnectionRecovery(lambda: self._reconnect_smb())
    ...
    def download_chunk_with_recovery(
        self, remote_path: str, offset: int, chunk_size: int, max_retries: int = 3
    ):
        """
